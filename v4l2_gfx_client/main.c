...
static struct test_v4l2_session* testcluster_vidioc_mmap(
         ...
         * object
         */
        sesh = calloc(1, sizeof(struct test_v4l2_session));
        assert(sesh);
        sesh->fd = fd;
        sesh->bcount = bcount;
        sesh->pix = *pix;
        sesh->bufs = calloc(bcount, sizeof(char*));
        assert(sesh->bufs);
        sesh->lens = calloc(bcount, sizeof(int));
        assert(sesh->lens);
        for (idx=0; idx<bcount; idx++) {
...
//	fmt.fmt.pix.width = 852; fmt.fmt.pix.height = 362;
        fmt.fmt.pix.width = param_width;
        fmt.fmt.pix.height = param_height;
