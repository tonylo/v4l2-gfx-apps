...
int (*ptr_init_glstate)(int fd);
void (*ptr_deinit_glstate)(void);
void (*ptr_stream_texture)(int dev, void* dev2texture);
void (*ptr_draw_frame)(int fd);
...
//XXX    if ((egl_err = eglChooseConfig(egldpy, config_attribs, configs, 1,
...
int EglMain(EGLNativeDisplayType eglDisplay, EGLNativeWindowType eglwindow)	
{
	...
	ptr_init_glstate = init_glstate;
	ptr_deinit_glstate = gles2_deinit_glstate;
	ptr_stream_texture = stream_texture;
	ptr_draw_frame = draw_frame;
	glesversion = 2;
    ...
    if ((rv = ptr_init_glstate(dev_fd)) != 0) {
		ERROR("ptr_init_glstate failed\n");
		goto end;
	}
	...
	ptr_stream_texture(0, (void*)dev2texture);

	while (1 && handle_events() == 0) {
        ptr_draw_frame(dev_fd);
    }
